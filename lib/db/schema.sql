-- People's Justice Database Schema
...
CREATE TABLE case_types (
  ...
  name VARCHAR(200) NOT NULL,
  category VARCHAR(50) NOT NULL, -- massAction, singleEventPI, institutionalAbuse, consumer, other
...
CREATE TABLE geos (
  id BIGSERIAL PRIMARY KEY,
  type VARCHAR(20) NOT NULL, -- state, county, city
  code VARCHAR(10), -- State code (CA, TX, etc.)
  name VARCHAR(200) NOT NULL,
...
CREATE TABLE firms (
  ...
  bar_states TEXT[], -- Array of state codes
  reviews_count INT DEFAULT 0,
  avg_rating DECIMAL(3,2),
  intake_sla_sec INT, -- Intake SLA in seconds
  active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE placements (
  id BIGSERIAL PRIMARY KEY,
  firm_id BIGINT NOT NULL REFERENCES firms(id) ON DELETE CASCADE,
  type VARCHAR(50) NOT NULL, -- featured, exclusive
  scope VARCHAR(50), -- case, injury, geo
  scope_id BIGINT, -- ID of case_type, injury, or geo
...
CREATE TABLE leads (
  ...
  phone VARCHAR(20),
  email VARCHAR(255),
  consent_snapshot JSONB NOT NULL, -- TCPA compliance: {text, timestamp, ip, userAgent, etc.}
...
CREATE TABLE buyers (
  id BIGSERIAL PRIMARY KEY,
  name VARCHAR(200) NOT NULL,
  criteria JSONB, -- Matching criteria {caseTypes, injuries, geos, etc.}
  webhook_url VARCHAR(500),
  sla_sec INT, -- SLA in seconds
...
CREATE TABLE router_logs (
  ...
  attempts JSONB, -- Array of routing attempts with timestamps
  outcome VARCHAR(50),
...
CREATE TABLE lead_audit_log (
  id BIGSERIAL PRIMARY KEY,
  lead_id BIGINT NOT NULL REFERENCES leads(id) ON DELETE CASCADE,
  event_type VARCHAR(50) NOT NULL, -- created, viewed, contacted, routed, accepted, rejected
  actor VARCHAR(100), -- 'system', 'user:{id}', 'buyer:{id}'
  metadata JSONB,
  ip_address INET,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
COMMENT ON TABLE brands IS 'Multi-brand configuration';
COMMENT ON TABLE case_types IS 'Mass Action, MVA, Institutional Abuse, etc.';
COMMENT ON TABLE leads IS 'Lead intake records with TCPA consent snapshots';
COMMENT ON TABLE lead_audit_log IS 'Immutable audit trail for compliance';
COMMENT ON COLUMN leads.consent_snapshot IS 'TCPA compliance: full consent text, timestamp, IP, user agent, form version';
